...
# Journal access: the unit logs to the journal, and the diagnostic bundle
# collects it with `journalctl -u ucm`. Reading a system unit's journal needs
# systemd-journal membership, which the service user had none of, so the
# bundle shipped without the journal it advertised.
if getent group systemd-journal >/dev/null 2>&1; then
    usermod -aG systemd-journal %{name} 2>/dev/null || true
fi
