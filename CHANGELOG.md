...
### Added
- The server's own application log can be read from the interface, under System Logs. It is the log that explains a failure no record was ever created for: a SCEP enrolment refused during validation is rejected before a request row exists, so it is written there and nowhere else, and reading it used to mean a shell on the host. Every filter is applied on the server, so none of them hides a line that was simply never fetched: the source, the subsystem that wrote the line, a level floor, a From/To window, a search, an exclusion and the number of lines, from 100 to 5000. Four sources are offered, each only where it exists: the application log, the gunicorn access and error streams, and the systemd journal. Live logs polls every five seconds. Secrets are redacted before anything leaves the process, a traceback stays one entry instead of one per line, and a line in no format the reader knows is shown without a level rather than dropped. Reading is restricted to administrators and is deliberately not audited, because the audit trail is written to this same log and the page polls; the diagnostic bundle download stays audited.

### Changed
- The service now runs with the `systemd-journal` supplementary group, granted in the unit file on both the DEB and the RPM. Without it the service user cannot read its own unit's journal, so the journal section of the diagnostic bundle was empty on every native install. The grant is scoped to the service and revoked by commenting out one line, rather than being added to the account for good.
...
### Fixed
- The application log was written to a file nothing could read back. A container logged to standard output only, and a native install whose `/var/log/ucm` could not be opened fell back to stderr, so on both the diagnostic bundle shipped without the application log its own description advertises. One file is now chosen at startup and recorded, so the bundle and the log viewer read the file the logging setup writes: the existing `/var/log/ucm/ucm.log` where it can be opened, and `DATA_DIR/ucm.log` otherwise, which in a container is the data volume, beside the standard output stream `docker logs` still shows. `UCM_LOG_MAX_BYTES` and `UCM_LOG_BACKUPS` set its rotation, ten megabytes over five generations by default.
- Most of UCM's own secrets were not redacted from the diagnostic bundle. The pattern asked for a word boundary, and there is none after an underscore, so `password=` was removed while `client_secret=`, `challenge_password=`, `bind_password=` and `smtp_password=` went out in full, as did a password written as JSON. Names the pattern did not know were missed whatever their shape: `secret_key`, `eab_hmac_key`, `user_pin`, `passphrase`. Cookie headers and the password in a URL's userinfo were not covered at all. The pattern is now checked against the names UCM's settings and its provider credential schemas actually use, in the query-string, JSON and quoted forms, and a quoted value is taken whole rather than up to its first space. Settings that only read like secrets, such as `password_set`, `min_password_length`, `token_label` and `token_url`, stay legible. The log viewer shares the same pass.
