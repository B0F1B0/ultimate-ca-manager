...
### Security
- Exporting a user certificate handed over its private key to anyone who could see the certificate. The route that exports an enrolment asked for no particular permission before including the key, and included it by default, while the two other export routes require a separate administrator-only permission for exactly that and say why: without it the approval-gated key recovery flow is pointless, because anyone who could ask for a recovery could export the key instead and skip the approval. The built-in Operator role holds the permission to read user certificates and not the one for private keys, and its access check reaches every user's enrolment, so one request returned any user's private key in a password-protected bundle. Taking somebody else's key now requires the private-key permission on this route as on the others. Taking your own is unchanged: a person downloading their own enrolment from their own account page still gets their key with it, which is what that page is for
- Audit entries written outside the audit service were never sealed. Eight places built one by hand and added it to the database themselves, because they were in the middle of a transaction they meant to commit on their own terms and the service would have committed it for them: certificate enrolment over two protocols, the CRL regeneration setting, automatic renewal failures, responder certificate renewal, and the bundle import. Each entry has a hash of its own and of the one before it, which is what makes the trail a chain rather than a list, and all eight left both empty. Worse, the integrity check stepped over an entry carrying no hash and restarted its chain there, so it reported a valid ledger straight across every gap, and a row inserted or altered between two unsealed ones could not be told from a genuine record. All eight now go through one place that seals the entry and leaves the commit to the caller, a test refuses the next one written by hand, and the integrity check reports what it cannot attest instead of passing over it
- An audit entry commits the session it is written on, so any entry written before the change it describes was the call that decided whether that change survived. Twenty-three places had it that way and were corrected one at a time, and a test now holds the order. That test had to be rewritten twice before it earned the claim: it began by recognising only two ways of staging a change and caught nine of the sixteen it was written for, and it read an audit entry only when it was written as a direct call, so half the refusals in this codebase, which record themselves through a local helper, were invisible to it. It still does not follow a change staged inside a function the route calls, and three refusals of the same family remain open for that reason, one of them reachable by a certificate client. Correcting the places it does cover turned up several the review had not: Updating a Microsoft CA connection applies the submitted fields to the record one by one, so a value of the wrong type part way down raised and the route answered 500 with the earlier fields still pending: auditing that refusal is what committed them, and a request carrying a new name and a new password was refused and kept both. Attaching an externally issued certificate to its pending request, when done as part of a bundle import, recorded itself with an audit entry that committed the importer's entire bundle rather than leaving it to the importer, and undid the whole bundle when that entry could not be written. A certificate deployed to a remote host recorded the delivery the same way, after the file was already in place: an entry that could not be written rolled the delivery record back, and the next pass pushed the same certificate again. Deleting a certificate authority now records the failure as well as the success, since its key and certificate are removed from disk before the row and a failed deletion otherwise left nothing to say so, and a deletion is no longer credited to an internal caller when it was an operator's request that triggered it. The database reset endpoint imported a function that does not exist, so it answered 500 and reset nothing on every call; it works again, and records itself in the table it recreates rather than in the one it is about to destroy, naming whoever asked for it rather than looking them up in the table that has just been emptied. Creating or updating a Microsoft CA connection applies the submitted fields one at a time, and both routes recorded a refusal while those fields were still pending, which is what saved them: a request carrying a new name and a new password was refused and kept both. Creating a certificate authority whose CRL distribution point could not be set left that change pending and silent, for the next call to commit the session to carry. A certificate revocation the server refuses, over the certificate management protocol, no longer revokes the certificate: the revocation is staged in one transaction and the entry recording the refusal was what committed it, so a client was answered that the revocation had failed while the certificate was revoked and its serial published on the revocation list. Signing a request through a Microsoft CA, and updating a hardware security module provider, each recorded their own refusals the same way, keeping a record half rewritten and a provider renamed on calls answered as failures. Restoring an archive that carries no data no longer writes an entry saying it was restored beside a response saying it was not, and the archive's name now reaches the ledger from either page instead of only one. Everything a restore does once the archive has been read is now shared between the two pages rather than written twice: recording it, which on one page could turn a restore that had succeeded into a reported failure; asking the service to restart, which one page did and the other left to the operator while telling them it had been done; and handing back what the archive said about itself, which reached a client from one page only. The restart was also reported as requested whenever it was attempted, whether or not it had been accepted. And a restore no longer applies a password length rule that belongs to creating an archive: an archive made before that rule existed, or by another installation, could not be restored from one of the two pages at all. The two single sign-on callbacks record their own failures the same way, so a half-made account and the identity binding that goes with it are no longer written by the entry that reports the login failing
- A single sign-on login for an account an administrator has disabled no longer writes to that account on its way to being refused. The directory's email, full name and role were applied first and the refusal came afterwards, so every attempt re-synchronised a record nobody can use and recorded a role change for a login that did not happen. The one that outlived the attempt is the stable external identifier, which is bound once and decides whose account it is: bound while the account was off, it would have handed that account to whoever held the identifier at the directory on the day it was switched back on
- Two cloud metadata endpoints were missing from the guard that every outbound request goes through, so they were reachable from each of them: ACME, webhooks, SSO, the update check and the appliance import. Oracle Cloud's endpoint sits inside a range the guard treats as private, and private addresses are allowed on purpose because an on-premises installation is pointed at them all day, so it had to be named like the others. The second is the same address as the endpoint already blocked, written the way an IPv6-only network with a translating gateway writes it: the guard understood one IPv6 spelling of an IPv4 address and not that one. Both are refused now, and the translated form is read back to the address it carries rather than the whole range being refused, which would cut off ordinary traffic on such a network. Six ways of writing an IPv4 address as IPv6 reach the same host, and the guard understood one of them: the other five are read the same way, so a denied address cannot come back simply by being spelled differently. One byte of the translated form is reserved and a sender is told to leave it at zero, and the guard used to refuse to read any address where it was not: since the reading step in the specification removes that byte without checking it, a gateway forwards such an address just the same, and setting the byte was enough to switch the whole check off for the address that carried it. It is now read like the rest. The local-use translation prefix needed more care than the rest. It is deliberately shorter than any prefix an operator cuts out of it, so several layouts fit and the address alone does not say which one is in use. Reading it with a single layout was wrong in both directions: the metadata endpoint written for one of the other layouts came back as an unremarkable address and went through, while every ordinary host behind such a gateway came back as all zeroes and was refused. Every layout the specification allows is now read, and those readings are judged against the metadata endpoints alone, because a reading taken with the wrong layout lands on a loopback or an all-zero address constantly and refusing on that would cut off the traffic this is meant to leave alone. Scaleway's endpoint was missing too, in both its forms
- The two OPNsense import routes checked one address against the deny-list and connected to another. The port came from the request body and was never examined, and it carries the rest of the URL's authority: a port written as `443@169.254.169.254` turns the host that was checked into a user name and the address that follows into the host that is reached. One request was enough, with no hostile name server and no redirect, and the routes are open to the operator role. The deny-list is now asked about the address that will actually be requested, a port must be a number between 1 and 65535, and a host that carries a port, a path or credentials is refused instead of producing a server error. The import also stops following redirects: the appliance answers JSON on its API, so a redirect there is not a normal condition, and walking it read the answer of a host nobody had checked as if it were the appliance's own inventory of certificates. A redirect is now reported with the address it pointed at, which an installation behind a reverse proxy needs in order to understand the refusal. The host is converted to the form the client will resolve before anything is checked: the deny-list resolves names through one conversion of international characters and the HTTP client through another, and the two do not agree, so a name could be checked as one host and reached as a different one. An address carrying a zone identifier is refused for the same reason: a zone names an interface of the server itself, two addresses differing only by it are not equal, and the client strips it before resolving, so the address checked was not the address reached. A host is required to be written with the characters a host is written with, and one that is not text at all is refused by name instead of producing a server error. The name is resolved once, when it is checked, and the connection is held to the addresses that were checked: resolving again at connection time left a name answering different addresses in turn free to be checked as one host and reached as another, with no race to win
- Restoring a backup from the settings page skipped two refusals the same operation makes from the system page. It did not take the lock a backend migration holds, although a restore rewrites the whole database a migration is reading through, and it did not decline while a backend switch was written and waiting for the restart that applies it, in which case the restore lands on the backend being left behind and the restart throws it away. Which of the two pages an operator happened to use decided whether their restore survived; both now refuse in the same cases, with the same answer. The settings page also stops accepting a `mode` it does not honour: asking it to merge an archive replaced the whole instance instead, silently, while the same request to the system page merged as asked. An archive holding nothing but its own metadata is no longer answered as a restore there either: it revoked every session that was open, the caller's included, for a file that touched not one row. And both pages now repeat the two warnings an archive can carry, that it holds sections this version does not restore and that a record's stored private key is not its certificate's, which only one of them did: an authority in that state signs certificates nobody can verify, and the operator was told or not depending on which page they had used
- Deleting a local ACME account left its authorizations and its challenges behind on SQLite, and failed outright on PostgreSQL. Two of the three deletions compared the identifiers the protocol puts in URLs, which are text, to the table's own numeric keys: the question asked was whether a token equals a number. One database answered no and removed nothing, leaving rows pointing at an account that was gone; the other has no way to compare the two and refused the statement, so the account was not deleted either. Both now match on the identifier, as the route that removes a single order always did, and an authorization is found whether it names the account or only the order it belongs to, since the column is optional and the protocol code already reads it that way, and the account's rows are found by the account they name rather than by walking its orders: an authorization asked for before any order exists, which the protocol allows, belonged to no order and was left behind either way, and an order placed at an external authority went on naming an account that no longer existed
- A zone served by the built-in ACME server could be bound to an authority that cannot sign. The table that maps a zone to a DNS provider refuses six reasons an authority is unusable; the one that maps a locally served zone checked two of them, so an authority that is revoked, that sits under a revoked one, or that has been taken offline was accepted. Nothing was ever signed by it, but issuance refused the order without failing it, so the client retried for ever and nothing told the operator what was wrong. Both tables now ask the same question, and both stop asking it once the zone is saved: an authority taken offline afterwards would otherwise leave the zone impossible to correct or to switch off
- Testing whether an OAuth2 authorization endpoint is reachable followed redirects, and the address was resolved a second time by the connection. The check that refuses cloud metadata saw only the address that was typed: a redirect went wherever it pointed, and a name answering differently between the check and the connection did the same. The test now connects to the addresses that were checked and does not follow redirects, which a reachability test does not need since an answer of any kind already proves the endpoint responds. The status reported back is therefore the first answer rather than the end of a redirect chain, which for an identity provider that redirects reads as a 3xx where it used to read as the code of the page it ends on; the test is still reported as a success either way
- An administrator who was not the person who asked for a key recovery could not collect the key, although the rule is "the requester or an administrator" and the interface offered them the action. Two functions of the same name take their arguments in opposite orders, and this route called one the way the other expects, so the administrator half of the rule answered no to everyone, without raising. Nobody gained access they should not have had: what was lost is the recovery path itself, with the record of who collected the key and when
- Seven listing routes did not bound the page size they were asked for. Three put no ceiling on it at all: the audit log, the account activity log and the backup history took the number from the query string and passed it to the query, so one request for a million rows loaded the whole table into memory and serialised it. The account activity log is reachable by any signed-in account; the other two require an administrator. The remaining four had a ceiling and no floor, which is not a bound: a negative page size went through untouched, and a negative row limit means no limit at all on SQLite while PostgreSQL refuses the statement, so the same request returned the entire table on one backend and an error on the other. One of those four is the listing the audit page itself uses. All seven now go through the shared helper, which clamps at both ends while leaving each listing its own default, since a log line and a certificate are not the same density. A test refuses the next route that reads a page size without bounding it both ways
- Deleting a certificate could leave the row behind with its files already gone. The references other tables hold on it are detached first, then the files are removed, then the deletion was recorded, and recording it commits the session and rolls all of it back on failure: the references came back after the disk had been changed, and the delete that followed ran into them. On a database that enforces its foreign keys the certificate then survived with no certificate, no request and no key beside it. The deletion is recorded once it has committed
- Changing a certificate authority's CRL schedule, full or delta, was announced as done when the audit entry describing it could not be written. That entry commits the session it is given and rolls all of it back on failure, so written first it decided whether the schedule survived: the change was undone, the commit that followed committed nothing and reported success, and the answer carried the values that had just been lost. Both routes now save first and record afterwards, and so do the three other places with the same inversion: generating a CRL or a delta CRL, and installing an externally signed one, each handed back an object describing work the audit entry had just undone. Two ACME paths did the same, returning to the client a reference to an authorization that was no longer there. Synchronising a role from an identity provider recorded the change before making it. And refusing a change to a webhook wrote down everything the request carried before the part that was refused: an update answered as rejected moved the endpoint's address and replaced its signing secret anyway, because recording the refusal is what committed them
- Creating or changing a deployment target was announced as done when the audit entry that describes it could not be written. Writing that entry commits the session it is given and rolls all of it back on failure, so called before the target was committed it decided whether the target survived: the row was undone, the commit that followed committed nothing and reported success, and the answer carried an identifier for a target that is not there. Every route of that page now writes the row first and records it afterwards, which is the order two of them already documented: deleting a target, deleting a binding and running a connection test had the same inversion, the last one discarding the host key it had just pinned
- The two routes that delete a signing request would delete an issued certificate just as readily. A request and a certificate are the same table and the same counter of identifiers, and these routes deleted by identifier with no check that the row was a request, so they bypassed the refusal the certificate route gives for a certificate that is still valid and not revoked. They also ask for a different permission, one the operator role holds while it does not hold the right to delete certificates: the refusal that role is given on one route was handed to it on the other, and a valid certificate could leave the instance without the revocation list or the responder ever mentioning it. Both routes now act only on rows that are requests
- Applying a managed certificate to HTTPS from the administration interface wrote the stored private key to disk without decrypting it. On an installation with a key-encryption key, the file the server reads its key from received the ciphertext instead, over a backup of the working one, and the service was then asked to restart: HTTPS did not come back, while the answer read "Certificate applied". The renewal path had always decrypted it; both now read the key the same way, and a key that cannot be read refuses the change instead of writing something unusable
- Eight routes announced a success for a write the database had refused. `safe_commit` answers a pair, whether the commit went through and the response to send when it did not, and these eight tested the pair itself: a pair is always true, so the error branch could never run. The commit failed, the session was rolled back, and the route carried on. Creating an external-account binding answered 201 with no identifier and with the one-time HMAC key of a credential that does not exist, so the operator configured a client with a secret that opens nothing and the real one was never issued; the audit trail recorded the creation that had just been undone; and making external-account binding mandatory was answered as saved while ACME registration went on accepting accounts without one. Creating a user, deleting users in bulk and every change to an external-account credential were answered the same way. A test now refuses the next call site written that way
