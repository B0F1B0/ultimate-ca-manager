...
- Three listing routes returned as many rows as they were asked for. The audit log, the account activity log and the backup history took a page size from the query string and put no ceiling on it, so a single request for a million rows loaded the whole table into memory and serialised it; two of the three are reachable by any signed-in account. All three now go through the shared helper, which caps the page at a hundred rows while leaving each listing its own default, since a log line and a certificate are not the same density. A test refuses the next route written without a ceiling
