...
### Fixed
- Redacting a private key from the logs took a time that grew with the square of the text on a log seeded with unclosed `BEGIN` markers, which the SCEP endpoint lets anyone write there through a User-Agent, and the log viewer runs it on the whole tail. The block is now found in one pass, and the User-Agent is logged bounded.
...
- In the log viewer, an end bound given to the minute excluded the minute itself but for its first second, a slow answer could land over the filters chosen since it was asked, ticked rows kept their tick across a change of data, and a copied line of the access, error or journal source was rewritten in the application log's shape. The end bound closes its minute or day, only the newest request fills the page, the tick is cleared with the data, and a copy is the line as the log wrote it.
