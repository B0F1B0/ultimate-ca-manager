...
- A SCEP request reusing an already-issued transactionID with a different key was answered with the certificate of the first key, which the device cannot pair with its own. It is refused with `badRequest` now, and the log names the transaction.
