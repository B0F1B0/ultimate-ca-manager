...
### Fixed
- The Create CA wizard pre-ticked the serverAuth Extended Key Usage on every intermediate CA, so a CA created from the interface still restricted the chains beneath it to TLS servers, the defect v2.197 had closed on the API path only: an OpenSSL-based relying party such as FreeRADIUS rejects a client certificate issued under such a CA. The box now starts unticked (#228, reported by @stefanelul2000).
